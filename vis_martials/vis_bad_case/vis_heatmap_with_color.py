...
def apply_colormap_to_mesh(mesh, distances, colormap='jet', vmin=None, vmax=None, threshold=0.5):
    ...
    heatmap_colors = cmap(1.0 - normalized_distances)[:, :3]

    # 获取原始颜色，如果没有则使用灰色
    if mesh.has_vertex_colors():
        final_colors = np.asarray(mesh.vertex_colors).copy()
    else:
        final_colors = np.full((len(distances), 3), 0.7)  # 灰色

    # 只更新在阈值范围内的顶点颜色
    if np.sum(mask) > 0:
        final_colors[mask] = heatmap_colors[mask]
